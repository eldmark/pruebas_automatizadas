...
test.describe('RF-4.1 - Renovación rápida sin sesión (requisito NO implementado)', () => {
  ...
  test('TC-XX: No existe ningún enlace/botón de "renovación" en el sitio', async ({ page }) => {
    await page.goto(URL);
  ...
  test('TC-XX: Buscar un dominio ya registrado solo da opción de "Reservar", no "Renovar"', async ({ page }) => {
    await page.goto(URL);
    await cerrarModalPruebas(page);
  ...
  test('TC-XX: Comportamiento es idéntico para dominio existente y ficticio (ambos van a flujo de compra)', async ({ page }) => {
    await page.goto(URL);
    await cerrarModalPruebas(page);
